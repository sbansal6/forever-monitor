...
exports.attach = function (options) {
  ...
  if (options.emailConfig){
     if (fs.existsSync(options.emailConfig)) {
         try {
           monitor.emailConfig = JSON.parse(fs.readFileSync(options.emailConfig));
           monitor.transporter = nodemailer.createTransport(smtpTransport(monitor.emailConfig));
         } catch (ex){
             console.log('warn',ex)
         }

     } else {
       console.log('warn','not a valid emailConfig file')
     }
  }
  ...
  function sendMail(text) {
        var message = {
            // sender info
            from: monitor.emailConfig.from || os.hostname(),
            // Comma separated list of recipients
            to: monitor.emailConfig.to,
            // Subject of the message
            subject: "forever process restarted",
            // plaintext body
            text: text
        };
        monitor.transporter.sendMail(message, function (err) {
            if (err) {
